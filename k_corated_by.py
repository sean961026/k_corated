from rs import supp_user, pd_rating, dump, load, unknown_rating, dataset_choices, get_ratings_name_from_dataset, \
    get_all_web_files
        ...
        for item_id in items_need_to_rate:
            for record in part_ratings:
                if record[item_id] == unknown_rating:
                    record[item_id] = adapter(
                        pd_rating(original_ratings, int(record[-1] - 1), item_id, web)[0])
    else:
        for item_id in items_need_to_rate:
            if item_id not in items_to_keep:
                ...
                for record in part_ratings:
                    if record[item_id] == unknown_rating:
                        record[item_id] = adapter(
                            pd_rating(original_ratings, int(record[-1] - 1), item_id, web)[
                                0])
...
def get_k_corated_name_by_attr(dataset, k, web_name):
    web_name = web_name[4:-4]
    name = 'kratings_%s_%s_%s.csv' % (k, dataset, web_name)
    return name


def get_k_corated_index_by_attr(dataset, k, web_name):
    web_name = web_name[4:-4]
    name = 'index_%s_%s_%s.csv' % (k, dataset, web_name)
    return name
